...
import type { TripLink, TripLinkType } from '../types';
...
export const EditLinkPage: React.FC = () => {
  const { id: tripId, linkId } = useParams<{ id: string; linkId: string }>();
  ...
  useEffect(() => {
    if (!tripId) return;
    apiClient.get<TripLink[]>(`/api/links/${tripId}`)
      .then(({ data }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.error ?? 'שגיאה בעדכון הקישור');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loadingLink) {
    return (
      <AppShell>
        <div className="flex justify-center py-20 text-neutral-400 text-sm">טוען...</div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto px-4 pt-4 pb-36">
        ...
        <h1 className="text-2xl font-bold text-neutral-900 mb-6">עריכת קישור</h1>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div>
            <label className="text-sm font-bold text-neutral-700 mb-1.5 block">קובץ מצורף</label>
            {fileUrl ? (
